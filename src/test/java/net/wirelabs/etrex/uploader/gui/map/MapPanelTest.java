...
class MapPanelTest extends BaseTest {
    ...
    void shouldChangeRouteColorProcessEvents() {
        ...
        // then
        Awaitility.waitAtMost(Duration.ofSeconds(3)).untilAsserted(() -> assertThat(mp.routePainter.routeColor).isEqualTo(newRouteColor));
    }
    ...
    void shouldChangeRouteWidth() {
        ...
        Awaitility.waitAtMost(Duration.ofSeconds(3)).untilAsserted(() -> assertThat(mp.routePainter.routeLineWidth).isEqualTo(newLineWidth));
    }
    ...
    void shouldResetMap() {
        ...
        Awaitility.waitAtMost(Duration.ofSeconds(2)).untilAsserted(() -> assertThat(mp.routePainter.getObjects()).isNotEmpty());
    ...
    private void checkCoords(Collection<Coordinate> coords) {
        Awaitility.waitAtMost(Duration.ofSeconds(3)).untilAsserted(() -> assertThat(mp.routePainter.getObjects()).isNotEmpty());
        for (Coordinate coord : coords) {
            boolean contains = mp.routePainter.getObjects().stream()
