...
public class ApiUsagePanel extends BaseEventAwarePanel {
    ...
    public ApiUsagePanel(AppConfiguration configuration) {
        ...
        add(header, cc().cell(0,1,2,1).growX().alignY("center"));

        JSeparator separator = new JSeparator();
        add(separator, cc().cell(0,0,2,1).grow());

        JLabel dailyLabel = new JLabel("Daily:");
        add(dailyLabel, cc().cell(0,2).alignX("left"));

        dailyLimits = new JLabel("$daily");
        add(dailyLimits, cc().cell(1,2).alignX("right"));

        JLabel quarterLabel = new JLabel("15min:");
        add(quarterLabel, cc().cell(0,3).alignX("left"));

        quarterLimits = new JLabel("$15min");
        add(quarterLimits, cc().cell(1,3).alignX("right"));
    }
