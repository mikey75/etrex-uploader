package net.wirelabs.etrex.uploader.gui.desktop.stravapanel.activitiestable.details;
...
public class PhotosPanel extends BasePanel {
    ...
    private static BufferedImage getImageFromUrl(String url) throws IOException {
        ...
        if (StravaActivitiesPanel.getPhotoCache().get(url) == null) {
            // since images are on cloudfront - don't need strava credentials, just get the images on urls
            img = ImageIO.read(new URL(url));
            // cache the image
            StravaActivitiesPanel.getPhotoCache().put(url,img);
        } else {
            // it is cached, get it from cache
            img = StravaActivitiesPanel.getPhotoCache().get(url);
        }
        return img;
    }
}
